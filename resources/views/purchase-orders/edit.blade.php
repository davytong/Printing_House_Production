@extends('layouts.app')
@section('title','កែប្រែ — '.$purchaseOrder->po_number)
@section('page-title','Edit Purchase Order')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    <h1 class="section-title">កែប្រែ: {{ $purchaseOrder->po_number }}</h1>
        ...
        <div class="row g-3">
          ...
          <div class="col-md-3">
            ...
            <select name="currency" class="form-select">
              ...
              <option value="KHR" {{ old('currency',$purchaseOrder->currency)==='KHR'?'selected':'' }}>KHR</option>
            </select>
          </div>
          <div class="col-md-9">
    ...
    <div class="panel">
      ...
      <div class="panel-body">
        <div id="itemsContainer">
          ...
          <div class="item-row mb-3 p-3"
               style="background:var(--surface-2);border-radius:var(--radius);border:1px solid var(--border)">
            <div class="row g-2 align-items-end">
              ...
              <div class="col-md-1">
                ...
                <div class="form-control form-control-sm line-total"
                     style="background:var(--surface);font-family:var(--font-latin);
            ...
            <div class="row g-2 mt-1">
              <div class="col-md-4">
                <select name="items[{{ $i }}][inventory_item_id]"
                        class="form-select form-select-sm" style="font-size:.75rem">
        ...
        <div class="d-flex justify-content-end mt-2">
          <div style="font-family:var(--font-latin);font-size:1rem;font-weight:700;color:var(--primary)">
            Total: <span id="grandTotal">{{ number_format($purchaseOrder->total_amount,2) }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="panel">
      <div class="panel-body d-flex flex-column gap-3">
        <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:var(--radius);
          ...
          គ្រាន់តែ <strong>Draft</strong> PO ប៉ុណ្ណោះអាចកែបាន
        </div>
...
const invOpts = `@foreach($inventoryItems as $inv)<option value="{{ $inv->id }}">{{ $inv->name }} ({{ $inv->code }})</option>@endforeach`;
const unitOpts = `@foreach(['pcs','kg','liter','ream','box','roll','sheet'] as $u)<option value="{{ $u }}">{{ $u }}</option>@endforeach`;
...
function calcGrand() {
  let t = 0;
  document.querySelectorAll('.line-total').forEach(el => t += parseFloat(el.textContent) || 0);
...
document.getElementById('addItemBtn').addEventListener('click', () => {
  ...
  <div class="item-row mb-3 p-3" style="background:var(--surface-2);border-radius:var(--radius);border:1px solid var(--border)">
    <div class="row g-2 align-items-end">
      <div class="col-md-4"><label class="form-label" style="font-size:.75rem">ឈ្មោះ *</label>
        <input type="text" name="items[${idx}][item_name]" class="form-control form-control-sm" required></div>
      <div class="col-md-2"><label class="form-label" style="font-size:.75rem">Unit</label>
        <select name="items[${idx}][unit]" class="form-select form-select-sm">${unitOpts}</select></div>
      <div class="col-md-2"><label class="form-label" style="font-size:.75rem">ចំនួន *</label>
        <input type="number" name="items[${idx}][quantity_ordered]" class="form-control form-control-sm qty" min="0.01" step="0.01" value="1" style="font-family:var(--font-latin)" required></div>
      <div class="col-md-2"><label class="form-label" style="font-size:.75rem">តម្លៃ/Unit</label>
        <input type="number" name="items[${idx}][unit_price]" class="form-control form-control-sm price" min="0" step="0.01" value="0" style="font-family:var(--font-latin)"></div>
      <div class="col-md-1"><label class="form-label" style="font-size:.75rem">សរុប</label>
        <div class="form-control form-control-sm line-total" style="background:var(--surface);font-family:var(--font-latin);font-weight:600;pointer-events:none">0.00</div></div>
      <div class="col-md-1 d-flex align-items-end">
        <button type="button" class="btn btn-ghost btn-sm remove-row" style="color:var(--danger)"><i class="bi bi-trash3"></i></button></div>
    </div>
    <div class="row g-2 mt-1">
      <div class="col-md-4"><select name="items[${idx}][inventory_item_id]" class="form-select form-select-sm" style="font-size:.75rem"><option value="">ភ្ជាប់ Inventory</option>${invOpts}</select></div>
      <div class="col-md-4"><input type="text" name="items[${idx}][notes]" class="form-control form-control-sm" placeholder="Notes..." style="font-size:.75rem"></div>
    </div>
  </div>`;
