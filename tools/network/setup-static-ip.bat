...
echo  =====================================================
echo.
echo  This script will assign a FIXED IP address (172.16.18.177)
...
echo        The app will run on: http://172.16.18.177
echo.
...
netsh interface ip set address name="%ETH_NAME%" static 172.16.18.177 255.255.248.0 172.16.16.1
netsh interface ip set dns name="%ETH_NAME%" static 180.178.124.12
netsh interface ip add dns name="%ETH_NAME%" 8.8.8.8 index=2

echo.
echo  =====================================================
echo   DONE! Static IP has been set to 172.16.18.177.
echo.
...
powershell -Command "(Get-Content '.env') -replace 'APP_URL=.*', 'APP_URL=http://172.16.18.177' | Set-Content '.env'"
REM Also update client-setup.bat to have the correct IP
powershell -Command "(Get-Content 'tools\network\client-setup.bat') -replace 'set SERVER_IP=.*', 'set SERVER_IP=172.16.18.177' | Set-Content 'tools\network\client-setup.bat'"
php artisan optimize:clear >nul 2>&1
php artisan optimize >nul 2>&1

echo Laravel config updated to http://172.16.18.177.
echo.
pause
